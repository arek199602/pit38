...
- **Baza: PostgreSQL** (przeszliśmy z SQLite 2026-06-14, decyzja usera) — standard produkcyjny + odblokowuje porady Copelanda (rozdz. 14). Lokalnie Homebrew `postgresql@16` (socket /tmp:5432; **parytet z CI** `postgres:16`), bazy `pit38_development`/`pit38_test`. CI: serwis `postgres:16` + env `PGHOST/PGUSER/PGPASSWORD`. Lock ma platformy linux (prekompilowany `pg` na CI). `schema.rb` (format :ruby) łapie nasze CHECK constraints (Rails 7.1+ to umie) → `:sql` byłby TERAZ overkill. **Przełącz na `schema_format: :sql` dopiero, gdy dodasz feature PG, którego schema.rb NIE umie zapisać** (trigger, funkcja/procedura, indeks częściowy/wyrażeniowy, własny typ ENUM, exclusion constraint) — bo wtedy schema.rb po cichu go gubi i test DB ≠ produkcja. **Datetime = domyślny `timestamp` (bez strefy), świadomie NIE `timestamptz`** — user (8 lat Rails) nigdy tego nie potrzebował, dla niego overkill (PIT-38 = single-user, UTC). NIE podsuwaj tego ponownie.
- Gem **`strong_migrations`** (2026-06-15) — pilnuje, by migracje nie blokowały dużych tabel; w dev/CI flaguje niebezpieczne DDL i podpowiada bezpieczny zapis (np. `NOT VALID` + `validate_check_constraint` zamiast blokującego ADD). Config: `config/initializers/strong_migrations.rb` (`start_after` grandfatheruje istniejące migracje, `lock_timeout = 10s`).
